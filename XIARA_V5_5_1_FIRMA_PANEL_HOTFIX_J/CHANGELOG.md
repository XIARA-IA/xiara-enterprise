...
## V5.5.1 FIRMA PANEL HOTFIX J
- Reconstruido desde V5.5.0 STORAGE ENTERPRISE HOTFIX C para evitar acumulación de parches defectuosos.
- Añadido panel independiente de firma/sello al final del documento, insertado de forma segura antes de `</body>`.
- Eliminados los errores generados por inyección accidental dentro de strings HTML (`Invalid or unexpected token`, `missing ) after argument list`).
- Nuevas funciones globales: `xiaraFirmaDiagnosticoJ()`, `xiaraFirmaOpenJ()`, `xiaraSelloOpenJ()`.
- Los botones de firma y sello del panel de Firma Digital quedan interceptados para abrir el modal correcto.
