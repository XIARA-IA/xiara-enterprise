...
## V5.5.1 DOCUMENT/FIRMA/PRL HOTFIX E
- Corrige vista de firma personal en panel Firma Digital.
- Unifica almacenamiento de firma/sello entre raíz y empresa activa.
- Refuerza guardado de firma para usuario y firma empresa.
- Inserta firma y sello de empresa en vista previa PRL.
- Refuerza guardado de documentos PRL para conservar firma y sello en HTML.
- Al abrir documentos PRL guardados, prioriza HTML enriquecido con firma/sello sobre JPG antiguo sin sello.
- No cambia diseño, no borra módulos y mantiene compatibilidad V5.5.0.
...
## V5.5.1 DOCUMENT/FIRMA HOTFIX D
- Corregido `ReferenceError: h is not defined` al abrir documentos PRL / Salud laboral desde ficha de empleado.
- Añadido puente seguro entre el script principal `type=module` y parches clásicos antiguos.
- Expuestos de forma controlada `db`, `h`, `renderAll`, `modal`, `localSave`, `user`, `company`, `today`, `now` y helpers necesarios.
- Reforzado guardado de firma personal para que se vea inmediatamente en el panel tras guardar.
- Reforzada apertura de documentos PRL guardados con HTML/JPG/DataURL sin romper diseño ni módulos.
